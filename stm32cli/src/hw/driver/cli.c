  ...
  if (p_cli->state == CLI_RX_IDLE)
  {
    switch(rx_data)
    {
      ...
      default:
        ...
        {
          if (line->cursor == line->count)    //커서가 문자 끝에 있을 때
          {
          ...
          if (line->cursor < line->count)     //커서가 문자 중간에 있을 때
          {
            uint8_t mov_len;
...
bool cliRunCmd(cli_t *p_cli)
{
  ...
  if (cliParseArgs(p_cli) == true)
  {
    ...
    cliToUpper(p_cli->argv[0]);     // 대문자로 변경
  ...
  if (p_cli->state == CLI_RX_IDLE)
  {
    switch(rx_data)
    {
      ...
      case CLI_KEY_BACK:
        if (line->count > 0 && line->cursor > 0)
        {
          if (line->cursor == line->count)   // 커서가 맨뒤에 있는 경우
          {
            line->count--;
            line->buf[line->count] = 0;
          }

          if (line->cursor < line->count)    // 커서가 중간에 있는 경우
          {
            uint8_t mov_len;
      ...
      default:
        if ((line->count + 1) < line->buf_len)    // 커서가 맨뒤에 있는 경우
        {
          if (line->cursor == line->count)
          {
            uartWrite(p_cli->ch, &rx_data, 1);
